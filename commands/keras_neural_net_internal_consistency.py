...
for fold in folds:
  ...
  for iteration in range(10):
    ...
    predictions = model.predict(x_test, batch_size=128).transpose().tolist()[0]
  ...
  avg_conmat = {'tp': 0, 'tn': 0, 'fp': 0, 'fn': 0}
  for truth, guess in zip(y_test, [sum(a)/10 for a in np.array(prediction_set).transpose()]):
    if truth == 1 and guess > 0.5:
      avg_conmat['tp'] += 1
    elif truth == 1 and guess < 0.5:
      avg_conmat['fn'] += 1
      ...
      avg_conmat['tn'] += 1
  avg_conmats.append(avg_conmat)
  maj_conmat = {'tp': 0, 'tn': 0, 'fp': 0, 'fn': 0}
  for truth, guess in zip(y_test, [sum([round(el) for el in a])/10 for a in np.array(prediction_set).transpose()]):
    if truth == 1 and guess > 0.5:
      maj_conmat['tp'] += 1
    elif truth == 1 and guess < 0.5:
      maj_conmat['fn'] += 1
      ...
      maj_conmat['tn'] += 1
  maj_conmats.append(maj_conmat)
  print "MAJ CONMATS"
  print avg_conmats
  print "MAJ CONMATS"
  print maj_conmats

print "Conmat of averaging 10 models for 10-fold"
print json.dumps(merge_conmats(avg_conmats))
print "Accuracy of averaging 10 models for 10-fold"
print accuracy(merge_conmats(avg_conmats))
print "Sensitivity of averaging 10 models for 10-fold"
print sensitivity(merge_conmats(avg_conmats))
print "Specificity of averaging 10 models for 10-fold"
print specificity(merge_conmats(avg_conmats))

print "Conmat of majority 10 models for 10-fold"
print json.dumps(merge_conmats(maj_conmats))
print "Accuracy of majority 10 models for 10-fold"
print accuracy(merge_conmats(maj_conmats))
print "Sensitivity of majority 10 models for 10-fold"
print sensitivity(merge_conmats(maj_conmats))
print "Specificity of majority 10 models for 10-fold"
print specificity(merge_conmats(maj_conmats))